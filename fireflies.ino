...
void lightup(int red, int green, int blue, int time_ms)
{
  float rledval = red;
  float gledval = green;
  float bledval = blue;
  ...
  for(int i = 0; i < time_ms; i++)
  {
    ...
    analogWrite(ledBPort, bledval);
    delay(1);
    analogWrite(ledBPort, 0);
    
    analogWrite(ledGPort, gledval);
    delay(1);
...
void fade(int red, int green, int blue, int brightest_time)
{
  ...
  {
    lightup(red-(i-1), green-(i-1), blue-(i-1), i/6);
  }
  lightup(red, green, blue, brightest_time);
  for( int i = 1; i < 256; i*=2 )
  {
    lightup(red-i, green-i, blue-i, i/2);
  }
}
